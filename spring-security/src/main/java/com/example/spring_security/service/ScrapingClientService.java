...
public class ScrapingClientService {
    ...
    @Autowired
    public ScrapingClientService(@Qualifier("scraperRestTemplate") RestTemplate scraperRestTemplate, 
        ...
        log.info("Scraping Client Service configured with scraper base URL: {} and extended timeouts", this.scraperBaseUrl);
    }
    ...
    public void triggerImmobilierScrape() {
        ...
        try {
            ...
            ResponseEntity<Void> response = scraperRestTemplate.postForEntity(targetUrl, null, Void.class);
            ...
            if (errorMessage != null && errorMessage.contains("Read timed out")) {
                log.error("Immobilier scraping operation timed out at {}: {}. This is normal for large scraping operations.", 
                          targetUrl, e.getMessage());
                log.info("Note: Scraping may still be running in the background on the scraper module.");
            } else {
    ...
    public void triggerTayaraScrape(String url) {
        ...
        try {
            ...
            ResponseEntity<String> response = scraperRestTemplate.getForEntity(targetUri, String.class);
            ...
            String errorMessage = e.getMessage();
            if (errorMessage != null && errorMessage.contains("Read timed out")) {
                log.error("Tayara scraping operation timed out at {}: {}. This is normal for large scraping operations.", 
                          logUrl, e.getMessage());
                log.info("Note: Scraping may still be running in the background on the scraper module.");
            } else {
                log.error("Failed to send trigger request to Tayara scraper at {}: {}. Is the module running?", 
                          logUrl, e.getMessage());
            }
        } catch (Exception e) {
            String logUrl = (targetUri != null) ? targetUri.toString() : scraperBaseUrl + endpointPath + " (URI build failed)";
