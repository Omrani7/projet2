...
export class OAuth2CallbackComponent implements OnInit {
  ...
  completeAuthentication(token: string): void {
    // Save the token - this will update the auth state throughout the app
    this.authService.saveToken(token);
    console.log('OAuth2 login successful. Token saved.');
    
    // Get the original page the user was on when they clicked login
    const originalPage = localStorage.getItem('original_page');
    console.log('OAuth2Callback: Found original_page =', originalPage);
    ...
    // Hide loading indicator and role selection
    this.loading = false;
    this.showRoleSelection = false;
    
    // Small delay to let the UI update and token be processed
    setTimeout(() => {
      const targetUrl = originalPage || '/discovery';
      console.log('OAuth2Callback: Now navigating to:', targetUrl);
      
      // Replace the current history entry instead of adding a new one
      // This ensures that when back button is pressed, we skip the OAuth flow URLs
